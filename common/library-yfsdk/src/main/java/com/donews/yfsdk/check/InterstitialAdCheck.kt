...
object InterstitialAdCheck {
    ...
    fun isEnable(): AdCustomError {
        ...
        if (!AdConfigManager.mNormalAdBean.interstitial.enable) {
            return AdCustomError.CloseAdOne
        }

        return isCanShowInters()
    }

    private fun isCanShowInters(): AdCustomError {

        //安装时间
        val installTime = AppStatusUtils.getAppInstallTime()
        val duration = AdConfigManager.mNormalAdBean.interstitial.startTime * 60 * 1000
        if ((System.currentTimeMillis() - installTime) < duration) {
            Logger.d("插屏广告未超过设置开启时间")
        ...
        mmkv?.let {
            ...
            if (closeDuration > AdConfigManager.mNormalAdBean.interstitial.interval * 1000L) {
