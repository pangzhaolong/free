...
public abstract class BaseRecyclerViewAdapter<T, VH extends DataBindBaseViewHolder> extends RecyclerView.Adapter<VH> {
     ...
     * @param type
     */
    public void setData(int index, T data, Object type) {
        if (this.mDataList != null) {
            this.mDataList.set(index, data);
            this.notifyItemChanged(index + this.getHeaderViewCount(),type);
        }
    }
    ...
    public void removeData(int position) {
        if (this.mDataList != null) {
            ...
            this.notifyItemRangeChanged(position, getItemCount()); //刷新被删除数据，以及其后面的数据
        }
    }
     ...
     * 删除源数据后刷新该条数据
     */
    public void removeDataNotifyItem(int position) {
        if (this.mDataList != null) {
            this.notifyItemRemoved(position + this.getHeaderViewCount());
            this.notifyItemRangeChanged(position, getItemCount()); //刷新被删除数据，以及其后面的数据
        }
    ...
    public void onBindViewHolder(VH holder, int position) {
        ...
        switch (viewType) {
            ...
            case TYPE_LOADING_VIEW:
                break;
            default:
                int layoutPosition=holder.getLayoutPosition() - this.getHeaderViewCount();
                if(this.mDataList.size() >layoutPosition) this.convert(holder, this.mDataList.get(layoutPosition), position);
                break;
        }
    }
    ...
    public interface OnItemClickListener {
        void onItemClick(View view, DataBindBaseViewHolder viewHolder, int position);

        boolean onItemLongClick(View view, DataBindBaseViewHolder viewHolder, int position);
    }
     ...
     * @param viewType
     */
    private void initItemClickListener(final ViewGroup parent, final DataBindBaseViewHolder viewHolder, int viewType) {
