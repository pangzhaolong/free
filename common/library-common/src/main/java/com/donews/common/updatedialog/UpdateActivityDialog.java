...
public class UpdateActivityDialog extends BaseAppDialogActivity {
    ...
    public static void showUpdateDialog(Context context, ApplyUpdataBean bean) {
    ...
    public void initView() {
        if (getIntent() != null) {
            updateBean = (ApplyUpdataBean) getIntent().getSerializableExtra(UPDATE_ENTITY);
        }
        if (updateBean == null) {
            finish();
        }
    ...
    private void createdDownloadTask() {
        ...
        downloadManager = new DownloadManager(getApplicationContext(), getPackageName(), updateBean.getApk_url(), new DownloadListener() {
            @Override
