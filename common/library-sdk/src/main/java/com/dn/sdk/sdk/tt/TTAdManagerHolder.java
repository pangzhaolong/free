...
public class TTAdManagerHolder {
    ...
    private volatile static boolean sInit;

    public static void init(Context context, String appId, boolean openDebug) {
        doInit(context, appId, openDebug);
    }
    ...
    private static void doInit(Context context, String appId, boolean openDebug) {
        if (!sInit) {
            TTMediationAdSdk.initialize(context, buildConfig(context, appId, openDebug));
            sInit = true;
        }
    }

    private static TTAdConfig buildConfig(Context context, String appId, boolean openDebug) {
//
...
//                .build();
        ...
        return new TTAdConfig.Builder()
                .appId(appId) //必填 ，不能为空
                .appName(getAppName(context)) //必填，不能为空
                .openAdnTest(false)//开启第三方ADN测试时需要设置为true，会每次重新拉去最新配置，release 包情况下必须关闭.默认false
                .isPanglePaid(false)//是否为费用户
                .setPublisherDid(DeviceUtils.getMyUUID()) //使用SUUID
                .openDebugLog(openDebug) //测试阶段打开，可以通过日志排查问题，上线时去除该调用
                .usePangleTextureView(true) //使用TextureView控件播放视频,默认为SurfaceView,当有SurfaceView冲突的场景，可以使用TextureView
                .setPangleTitleBarTheme(TTAdConstant.TITLE_BAR_THEME_DARK)
                .allowPangleShowNotify(true) //是否允许sdk展示通知栏提示
                .allowPangleShowPageWhenScreenLock(true) //是否在锁屏场景支持展示广告落地页
