...
public class SaleListAdapter extends RecyclerView.Adapter<SaleListAdapter.CrazyListViewHolder> implements View.OnClickListener {

    private final Context mContext;
    private final List<FactorySaleBean.ListDTO> mGoodsList = new ArrayList<>();
    private final GoodsClickListener mListener;

    public SaleListAdapter(Context context, GoodsClickListener listener) {
        mContext = context;
        mListener = listener;
    }

    @SuppressLint("NotifyDataSetChanged")
    public void refreshData(List<FactorySaleBean.ListDTO> list, boolean isAdd) {
        if (!isAdd) {
            mGoodsList.clear();
        }
    ...
    public void onBindViewHolder(@NonNull CrazyListViewHolder holder, int position) {
        ...
        FactorySaleBean.ListDTO gi = mGoodsList.get(position);
        if (gi == null) {
            return;
        }
...
//        String.format("￥%d元券", gi.getCouponPrice());
    ...
    private SpannableString getTitleString(FactorySaleBean.ListDTO goodsInfo) {
        String span = "d ";
