    ...
    public static boolean jump(Activity activity, Notify2DataConfigBean.UiTemplat uiTemplat) {
        ...
        if (scheme.startsWith(SchemeHttp)) {
            ...
            ARouter.getInstance().build(RouterActivityPath.Web.PAGER_WEB_ACTIVITY)
                    ...
                    .withString("url", action)
                    .navigation();
            return true;//处理掉http相关协议的跳转
            ...
            try {
                ...
                if (actPageCzz == null || actPageCzz.isEmpty()) {
                    if (BuildConfig.DEBUG) {
                        ToastUtil.show(activity, "未找到跳转的页面。请检查跳转参数配置");
                    }
                    return false;
                }
                Class<?> activityCzz = Class.forName(actPageCzz);
                Intent intent = new Intent(activity, activityCzz);
                if (buildParams(intent, uri)) {
                    activity.startActivity(intent);
                    return true;
                }
                if (BuildConfig.DEBUG) {
                    ToastUtil.show(activity, "跳转处理异常。请检查跳转参数配置");
                }
                return false;//参数处理错误了。上层自己处理
            } catch (Exception e) {
                if (BuildConfig.DEBUG) {
                    ToastUtil.show(activity, "点击跳转相关逻辑处理错误：" + e);
                    e.printStackTrace();
                }
                return false;
            }
