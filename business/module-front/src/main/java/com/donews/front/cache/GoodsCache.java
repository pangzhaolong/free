...
public class GoodsCache {
    ...
    private static String s_award_bean_path = "";
    private static String s_wallet_bean_path = "";
    ...
    private static String s_category_bean_path = "";

    public static void init(Context context) {
        s_base_cache_path = context.getDir("jdd", MODE_PRIVATE).getAbsolutePath();
        s_award_bean_path = s_base_cache_path + "/award_bean.dat";
        s_wallet_bean_path = s_base_cache_path + "/wallet_bean.dat";
        ...
        s_category_bean_path = s_base_cache_path + "/category_bean_.dat";
    }
...
//                Log.e("TAG", new File(s_banner_bean_path).getAbsolutePath() + "<---");
            ...
            if (c == LotteryCategoryBean.class) {
                ois = new ObjectInputStream(new FileInputStream(s_category_bean_path + ext));
                ...
                ois = new ObjectInputStream(new FileInputStream(s_award_bean_path + ext));
            } else if (c == WalletBean.class) {
                ois = new ObjectInputStream(new FileInputStream(s_wallet_bean_path + ext));
            }
            assert ois != null;
    ...
    public static synchronized void saveGoodsBean(Object object, String ext) {
        ...
        try {
            ...
            if (object instanceof LotteryCategoryBean) {
                fos = new ObjectOutputStream(new FileOutputStream(s_category_bean_path + ext));
                ...
                fos = new ObjectOutputStream(new FileOutputStream(s_award_bean_path + ext));
            } else if (object instanceof WalletBean) {
                fos = new ObjectOutputStream(new FileOutputStream(s_wallet_bean_path + ext));
            }
            //要使用writeObject
            assert fos != null;
